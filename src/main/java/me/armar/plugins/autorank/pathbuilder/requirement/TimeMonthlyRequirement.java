...
public class TimeMonthlyRequirement extends AbstractRequirement {
    int timeNeeded = -1;

    public TimeMonthlyRequirement() {
    }

    public String getDescription() {
        return Lang.TIME_MONTHLY_REQUIREMENT.getConfigValue(AutorankTools.timeToString(this.timeNeeded, TimeUnit.MINUTES));
    }
    ...
    public String getProgressString(UUID uuid) {
        ...
        try {
            playtime = this.getAutorank().getPlayTimeManager().getPlayTime(TimeType.MONTHLY_TIME, uuid).get();
